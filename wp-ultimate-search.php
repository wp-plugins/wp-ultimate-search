...
{
	public static $in_footer = true; // toggle whether or not to put the scripts and styles in the footer. requires wp_footer().
	...
	*/
	...
	public function register_scripts() {
		...
		wp_enqueue_script( 'usearch-script', WPUS_BASE. 'js/main.js', array( 'visualsearch' ),'', self::$in_footer);
		wp_localize_script( 'usearch-script', 'usearch_script', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'searchNonce' => wp_create_nonce('search-nonce') ) );
	...
	public function get_results(){
		...
		if($catid = get_cat_ID($searcharray[0]["category"])) {
			$keywords = $this->string_to_keywords($searcharray[1]["text"]);
		} else {
			$keywords = $this->string_to_keywords($searcharray[0]["text"]);
		}
		...
		if( $searcharray && $array = $wpdb->get_results($querystring)){
			foreach ($array as $item){
					$excerpt = $this->highlightsearchterms(wp_strip_all_tags($item->excerpt), $keywords);
					$catstring = $this->render_categories($item);
					echo '<div class="usearch-meta"><a href="'. get_permalink($item->ID) .'">'. $item->post_title . '</a> ' . $catstring . '</div><div class="usearch-excerpt">' . $excerpt . '</div>';
			}
		} else {
